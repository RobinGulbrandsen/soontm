...
var dateHandler = function dateHandler() {
  ...
  var setDateInputAsNow = function setDateInputAsNow() {
    var now = new Date();
  ...
  var convertToDateAsMiliseconds = function convertToDateAsMiliseconds(day, hour, min, sec) {
    var dayList = day.split("-");
    return new Date(dayList[0], dayList[1] - 1, dayList[2], hour, min, sec).getTime();
  };

  return { setDateInputAsNow: setDateInputAsNow, convertToDateAsMiliseconds: convertToDateAsMiliseconds };
};
...
var _jsDateHandler = require("./js/dateHandler");

var _jsDateHandler2 = _interopRequireDefault(_jsDateHandler);

(function () {
  var dateHandlerObj = (0, _jsDateHandler2["default"])();
  var rootUrl = "/page?";

  var initFormValues = function initFormValues() {
    dateHandlerObj.setDateInputAsNow();
  };
  initFormValues();

  var buildUrl = function buildUrl(text, date, stretched, color, protocol, image) {
    return rootUrl + "text=" + text + "&date=" + date + "&stretched=" + stretched + "&color=" + color + "&protocol=" + protocol + "&image=" + image;;
  };

  var validateForm = function validateForm(text, date, stretched, color, image) {
    return true;
  };

  var newPageForm = document.querySelector("#newPageForm");
  newPageForm.addEventListener("submit", function (event) {
    event.preventDefault();

    var displayText = event.target.elements.text.value;
    var dateAsMiliseconds = dateHandlerObj.convertToDateAsMiliseconds(event.target.elements.day.value, event.target.elements.hour.value, event.target.elements.min.value, event.target.elements.sec.value);
    var imageurl = event.target.elements.imageUrl.value;
    var urlArray = imageurl.split("://");
    var isStretched = event.target.elements.isStretched.checked;
    var backgroundColor = event.target.elements.backgroundColor.value;
